...
public class BotControl extends Activity implements TouchJoystick.JoystickListener, ZMQSubscriberThread.OnMessageListener {
    ...
    private JSONObject lidarCmdObj;
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        lidar = (RPLidar) findViewById(R.id.lidarView);
        ...
        lidarCmdObj = makeCallReq("msp430", "get_rplidar", null, null);
    ...
    private void doLidar(final boolean block) {
        sendCommand(
                lidarCmdObj,
                block,
                new CommandReplyCallback() {
                    ...
                    public void onReply(final String reply) {
                        if (reply != null && parseCallReply(reply) != null) {
                            ...
                            try {
                                ...
                                JSONArray jsonPoints = dataObj.getJSONArray("data");
                                int[] pts = new int[360];
                                for(int i = 0; i < 360; i++) {
                                    pts[i] = (Integer)jsonPoints.get(i);
                                }
                                //txtConsole.setText(pts.toString());
